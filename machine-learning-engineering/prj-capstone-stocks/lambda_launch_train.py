...
def lambda_handler(event, context):
    ...
    print(filename)
    dataset = filename.split('_train')[0].replace('_', '-')
    print(dataset)
    ...
    training_job_params = {
        ...
        'TrainingJobName': dataset,
        'HyperParameters': {
            'filename': json.dumps(filename),
            'target': 'Adj_Close',
            'sagemaker_container_log_level': '20',
            'sagemaker_enable_cloudwatch_metrics': 'false',
            'sagemaker_program': 'train-infer.py',
            'sagemaker_region': os.environ['AWS_REGION'],
            'sagemaker_submit_directory': 's3://' + bucket + '/source/train-infer.tar.gz',
            's3-output': os.environ['S3_OUTPUT_PATH']
        },
        'AlgorithmSpecification': {
            'TrainingImage': '763104351884.dkr.ecr.' + os.environ[
                'AWS_REGION'] + '.amazonaws.com/mxnet-training:1.6.0-cpu-py3',
